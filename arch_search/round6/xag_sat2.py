...
def build(points, allowed, n, k, maxfan=None, cap=None):
    vid = [0]
    ...
    for i in range(k):
        ...
        # symmetry: the highest selected variable of A is strictly lower than the highest of B
        m = n + i
        for u in range(m):
            for v in range(u + 1):
                # forbid: top(A) = u  and top(B) = v with v <= u
                cls.append([-SA[i][u]] + [SA[i][w] for w in range(u + 1, m)] + [-SB[i][v]] + [SB[i][w] for w in range(v + 1, m)])
        # each AND after the first must use the previous AND somewhere downstream is not required
    if cap is not None:
        from pysat.card import CardEnc
        for i in range(k):
            for sel in (SA[i], SB[i]):
                enc = CardEnc.atmost(lits=sel[:-1], bound=cap, top_id=max(abs(l) for c in cls for l in c))
...
if __name__ == '__main__':
    ...
    budget = int(sys.argv[4]) if len(sys.argv) > 4 and sys.argv[4] != '0' else None
    ...
    for k in range(k0, k1 + 1):
        t0 = time.time()
        cls, SA, SB, SO = build(pts, allowed, n, k, cap=cap)
