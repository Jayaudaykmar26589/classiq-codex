...
W=len(mod); cw=[W-3,W-2,W-1]
s=f"{NP} {W} 1 3 {L} {ml} {iters} 6 {seed} 0.02 {fixed}\n"+" ".join(map(str,cw))+"\n"+" ".join(map(str,want))+"\n"
if fixed: s+=" ".join(map(str,Lm))+"\n"
for t in mod+ro: s+=f"{t & ((1<<64)-1):x} {t>>64:x}\n"
out=subprocess.run(['./lanelev2'],input=s,capture_output=True,text=True).stdout.split('\n')
print(side,'nanc',nanc,'L',L,'maxlev',ml,'seed',seed,'fixed',fixed,out[0],flush=True)
if out[0].split()[1]=='0':
    hd=out[0].split(); json.dump({'ops':[list(map(int,l.split()[1:])) for l in out if l.startswith('OP')],'cw':[int(hd[5]),int(hd[6]),int(hd[7])],'nanc':nanc},open(f'lev2_{side}_{nanc}_{L}_{ml}_{seed}_{fixed}.json','w'))
