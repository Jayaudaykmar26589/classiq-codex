...
static int NP, W, R, NB, L, MAXLEV, FIXED, NT, ANYW=1, PAIR=1; static int bestcw[8]; static double lam;
...
static int mis_cw(TT *v, int *c3) {
  int cnt[64][16]; memset(cnt, 0, sizeof cnt); int nk[64]; memset(nk, 0, sizeof nk);
  for (int p = 0; p < NP; p++) { int k = 0; for (int b = 0; b < NB; b++) k |= getb(v[c3[b]], p) << b; cnt[k][want[p]]++; nk[k]++; }
  int c = 0;
  for (int k = 0; k < (1 << NB); k++) { if (!nk[k]) continue;
    if (FIXED) c += nk[k] - cnt[k][Lmap[k]];
    else if (PAIR) { int s2 = 0; for (int l = 0; l < 16; l++) s2 += cnt[k][l] * cnt[k][l]; c += (nk[k] * nk[k] - s2) / 2; }
    else { int m = 0; for (int l = 0; l < 16; l++) if (cnt[k][l] > m) m = cnt[k][l]; c += nk[k] - m; } }
  return c;
}
static int mis(TT *v) {
  if (!ANYW) return mis_cw(v, cw);
  int best = 1 << 30, c3[3];
  for (int a = 0; a < W; a++) for (int b = a + 1; b < W; b++) for (int d = b + 1; d < W; d++) {
    c3[0] = a; c3[1] = b; c3[2] = d; int m = mis_cw(v, c3); if (m < best) { best = m; bestcw[0] = a; bestcw[1] = b; bestcw[2] = d; } }
  return best;
}
...
int main() {
  ...
  { TT v[MAXW]; int lv[MAXW], na; run(best, L, v, lv, &na); mis(v); printf("RES %d %d %d CW %d %d %d\n", bw, bn, bl, bestcw[0], bestcw[1], bestcw[2]); }
