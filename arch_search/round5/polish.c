...
static int NP, W, R, L, NT, cw[3], want[128]; static TT init[20], ONE;
...
static int mis(TT *v) { int cnt[8][16]; memset(cnt, 0, sizeof cnt); int nk[8] = {0};
  for (int p = 0; p < NP; p++) { int k = getb(v[cw[0]], p) | getb(v[cw[1]], p) << 1 | getb(v[cw[2]], p) << 2; cnt[k][want[p]]++; nk[k]++; }
  int c = 0; for (int k = 0; k < 8; k++) { int m = 0; for (int l = 0; l < 16; l++) if (cnt[k][l] > m) m = cnt[k][l]; c += nk[k] - m; } return c; }
...
int main() {
  if (scanf("%d %d %d %d %d %d %d", &NP, &W, &R, &L, &cw[0], &cw[1], &cw[2]) != 7) return 1; NT = W + R;
